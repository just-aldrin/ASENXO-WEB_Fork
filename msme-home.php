...
<head>
  ...
  <style>
    ...
    .input-group label { display: block; font-size: 8px; color: #777; text-transform: uppercase; font-weight: 800; margin-bottom: 1px; }
    .input-group input { width: 100%; padding: 4px 8px; background: #000; border: 1px solid #333; color: white; border-radius: 3px; font-size: 11px; }
    ...
    .theme-btn:hover { border-color: #666; color: white; }
  </style>
</head>
<body class="dark">
<div class="app">
...
<script>
  ...
  function getForm(id) {
    ...
    if (id === 3 && profile) {
      ...
      <div class="form-grid">
        ...
        <div class="input-group"><label>Status</label><input id="o_mar" placeholder="Single/Married"></div>
    ...
    return `<p style="font-size:9px; color:#444;">Pending...</p>`;
  }
  ...
  async function save() {
    ...
    if (!error) next(); else alert(error.message);
  }
  ...
  function logout() { supabase.auth.signOut().then(() => window.location.href = 'login.php'); }
  window.onload = init;
</script>
</body>
</html>