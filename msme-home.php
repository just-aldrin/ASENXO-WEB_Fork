...
<head>
  ...
  <style>
    ...
    /* Layout Sizing - Ultra Squeezed */
    .user-profile-box { padding: 6px 10px; border-bottom: 1px solid #333; margin-bottom: 4px; display: flex; align-items: center; gap: 8px; }
    .user-avatar { width: 26px; height: 26px; background: var(--accent-color); border-radius: 4px; display: flex; align-items: center; justify-content: center; color: white; font-size: 10px; }
    .user-name-sidebar { font-weight: 600; font-size: 10.5px; color: white; }
    
    /* Step Items - Minimalist */
    .step-list { list-style: none; padding: 0; margin: 0; }
    .step-item { background: #1a1a1a; border: 1px solid #333; border-radius: 5px; margin-bottom: 4px; overflow: hidden; }
    .step-header { padding: 4px 10px; display: flex; align-items: center; gap: 8px; cursor: pointer; }
    .step-icon { width: 18px; height: 18px; border-radius: 50%; background: #262626; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 8.5px; color: #555; }
    ...
    .step-title { font-weight: 600; font-size: 10.5px; line-height: 1; margin-bottom: 1px; }
    .step-desc { font-size: 8.5px; color: #666; display: block; }

    /* Content Area */
    .step-content { display: none; padding: 6px 10px 10px 36px; border-top: 1px solid #262626; }
    .step-item.active .step-content { display: block; }

    /* Form Grid - High Density */
    ...
    .input-group input, .input-group select { width: 100%; padding: 3px 6px; background: #000; border: 1px solid #333; color: white; border-radius: 3px; font-size: 10px; }
    .input-group input:disabled { opacity: 0.4; border-style: dashed; }
    
    .primary-btn { background: var(--accent-color); color: white; border: none; padding: 5px; border-radius: 3px; cursor: pointer; width: 100%; font-weight: 700; margin-top: 4px; font-size: 10px; display: flex; align-items: center; justify-content: center; gap: 4px; }
    .spinner { animation: rotate 1s linear infinite; display: none; font-size: 8px; }
...
<div class="app">
  <div class="top-header">
    ...
    <div class="top-header-right">
      <button id="logoutBtn" style="background:#ef4444; color:white; border:none; padding:2px 6px; border-radius:3px; font-size:9px; cursor:pointer;">Logout</button>
    </div>
  </div>
  ...
  <div class="content-row">
    ...
    <div class="main-content">
      ...
      <div class="info-column">
        <div class="card">
          <div class="card-title" style="font-size: 10px;">Progress</div>
          <div class="info-stats">
            <div class="stat-item"><span class="stat-label">Status</span><span class="stat-value" id="progressPercent" style="font-size: 10px;">0%</span></div>
...
<script>
  ...
  // Initialization with explicit headers to solve "No API key found"
  ...
  const stepDetails = [
    { id: 1, title: "Account", desc: "Type selected" },
    { id: 2, title: "Verification", desc: "Identity check" },
    { id: 3, title: "Owner Profile", desc: "Detailed information" },
    { id: 4, title: "Photo ID", desc: "Upload identity" },
    { id: 5, title: "Business", desc: "Tax & Registration" },
    { id: 6, title: "Documents", desc: "Legal permits" }
  ];
  ...
  function getFormHtml(id) {
    if (id === 2) return `<button class="primary-btn" onclick="moveNext()">Continue</button>`;
  ...
  async function saveOwnerInfo() {
    ...
    if (!error) moveNext();
    else { alert(error.message); btn.disabled = false; spinner.style.display = 'none'; }
  }
